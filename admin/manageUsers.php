...
// Add this PHP code near the top of the file, after the database connection
...
// Add this PHP code near the top of the file, after the existing deleteUser handler
if (isset($_POST['resetPassword'])) {
    ...
    mysqli_stmt_bind_param($stmt, "si", $defaultPassword, $userId);
    
    $response = array();
    if (mysqli_stmt_execute($stmt)) {
        // Log the password reset in edit_history
        $logQuery = "INSERT INTO edit_history (user_id, action, item_type, item_id, changes) VALUES (?, ?, ?, ?, ?)";
        ...
        $itemType = "profile";
        $changes = json_encode(['password' => ['action' => 'reset']]);
        
        mysqli_stmt_bind_param($logStmt, "issis", $currentUserId, $action, $itemType, $userId, $changes);
        ...
        $response['success'] = false;
        $response['message'] = "Error resetting password: " . mysqli_error($conn);
    }
...
<body>
            ...
            <div class="topbar">
                ...
                <div class="profile-container">
                    <?php
                    // Fetch user data using the existing connection
                    if (isset($_SESSION['user_id'])) {
                        $userId = $_SESSION['user_id'];
                        $query = "SELECT id, name, profile_image FROM users WHERE id = '$userId'";
                        ...
                        if ($result && mysqli_num_rows($result) > 0) {
                            ...
                            $profileImage = !empty($user['profile_image']) && file_exists("../upload/profiles/" . $user['profile_image']) 
                                ? $user['profile_image'] 
                                : 'default.jpg';
                            ?>
                            <img src="<?php echo htmlspecialchars('../upload/profiles/' . $profileImage); ?>" alt="Profile Picture">
                            <span class="profile-name"><?php echo htmlspecialchars($user['name']); ?></span>
                            <?php
                        } else {
                            // Fallback if user data couldn't be fetched
                            ?>
                            <img src="../upload/profiles/default.jpg" alt="Default Profile Picture">
                            <span class="profile-name">Unknown User</span>
                            <?php
                        }
                    } else {
                ...
                <div class="content-card dashboard-table">
                    ...
                    <div class="table-container">
                        <table>
                            ...
                            <tbody>
                                <?php
                                // Fetch users with their group levels
                                $query = "SELECT u.*, ug.group_level 
                                          FROM users u 
                                          LEFT JOIN user_groups ug ON u.role = ug.group_name 
                                          ORDER BY u.id ASC";
                                $result = mysqli_query($conn, $query);
                                $counter = 1;
                                ...
                                if ($result) {
                                    while ($user = mysqli_fetch_assoc($result)) {
                                        // Convert numeric status to proper class and text
                                        $statusClass = $user['status'] == 1 ? 'status-active' : 'status-inactive';
                                        $statusText = $user['status'] == 1 ? 'Active' : 'Inactive';
                                        
                                        // Determine role display based on group_level
                                        $displayRole = '';
                                        if ($user['group_level'] == 1) {
                                            ...
                                            // Fallback to the actual role name if level is not 1 or 2
                                ...
                                ?>
                                        <tr>
                                            ...
                                            <td class="actions">
                                                <button class="action-btn edit" data-id="<?php echo $user['id']; ?>">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <button class="action-btn delete" data-id="<?php echo $user['id']; ?>">
                                                    <i class="fas fa-trash"></i>
                                                </button>
                                                <button class="action-btn reset-pwd" data-id="<?php echo $user['id']; ?>">
                                ...
                                <?php
                                    ...
                                    echo "<tr><td colspan='6'>Error fetching users: " . mysqli_error($conn) . "</td></tr>";
                                }
                                ?>
                            </tbody>
                        </table>
                    ...
                    button.addEventListener('click', function() {
                        ...
                        document.getElementById('confirmDelete').onclick = function() {
                            ...
                            .catch(error => {
                                ...
                                modal.style.display = 'none';
                                showAlert('Admin Account Can\'t Be Deleted', 'error');
                            });
                        };
